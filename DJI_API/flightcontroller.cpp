      ...
      command(0, 0, 0, 0, 0)
{
    ...
    connect(this, SIGNAL(changeCommandStatus(QString)),
                                      ...
                                      UserData userData)
{
    ...
    if (recvFrame.recvInfo.len - OpenProtocol::PackageMin <= sizeof(uint16_t))
    {
        ...
        if (ACK::getError(ack))
        {
            ...
            emit fc->changeCommandStatus("Error! See callback.");
        }
        else
        {
            emit fc->changeCommandStatus("Success");
        }
    }
    else
    {
...
void FlightController::commandStatusChanged(QString textToDisplay)
{
